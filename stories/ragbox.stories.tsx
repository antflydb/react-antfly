...
import { Antfly, RAGBox, Results } from "../src";
...
export const Basic = () => {
  return (
    <Antfly url={url} table={tableName}>
      ...
      <pre>{`<RAGBox id="question" fields={["TICO", "AUTR"]} />`}</pre>
      <RAGBox id="question" fields={["TICO", "AUTR"]} />
      <Results
        id="result"
        items={(data) =>
...
export const WithCustomPlaceholder = () => {
  return (
    <Antfly url={url} table={tableName}>
      <h1>RAGBox with Custom Placeholder</h1>
      <pre>{`<RAGBox
  id="question"
  fields={["TICO"]}
  placeholder="Ask about books..."
/>`}</pre>
      <RAGBox id="question" fields={["TICO"]} placeholder="Ask about books..." />
      <Results
        id="result"
        items={(data) =>
          data.map(({ _source: s, _id }) => (
...
export const WithCustomButtonLabel = () => {
  return (
    <Antfly url={url} table={tableName}>
      <h1>RAGBox with Custom Button Label</h1>
      <pre>{`<RAGBox
  id="question"
  fields={["AUTR"]}
  buttonLabel="Search"
/>`}</pre>
      <RAGBox id="question" fields={["AUTR"]} buttonLabel="Search" />
      <Results
        id="result"
        items={(data) =>
          data.map(({ _source: s, _id }) => (
...
export const WithInitialValue = () => {
  return (
    <Antfly url={url} table={tableName}>
      <h1>RAGBox with Initial Value</h1>
      <pre>{`<RAGBox
  id="question"
  fields={["TICO"]}
  initialValue="science fiction"
/>`}</pre>
      <RAGBox id="question" fields={["TICO"]} initialValue="science fiction" />
      <Results
        id="result"
        items={(data) =>
          data.map(({ _source: s, _id }) => (
...
export const WithOnSubmitCallback = () => {
  ...
  return (
    <Antfly url={url} table={tableName}>
      <h1>RAGBox with onSubmit Callback</h1>
...
export const WithCustomQuery = () => {
  ...
  return (
    <Antfly url={url} table={tableName}>
      <h1>RAGBox with Custom Query Function</h1>
...
<RAGBox id="question" customQuery={customQuery} />`}</pre>
      <RAGBox id="question" customQuery={customQuery} />
      <Results
        id="result"
        items={(data) =>
          data.map(({ _source: s, _id }) => (
  ...
  return (
    <Antfly url={url} table={tableName}>
      ...
      <style>{`
        ...
        .react-af-ragbox button:disabled {
          background: #ccc;
          cursor: not-allowed;
        }
      `}</style>
